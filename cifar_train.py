...
parser.add_argument('--dataset', metavar='DATASET',default='cifar100', type=str,
                    help='dataset name')

#Network
parser.add_argument('-a', '--arch', metavar='ARCH', default='layer13',
                    choices=model_names,
                    help='model architecture: ' +
                        ' | '.join(model_names) +
                        ' (default: layer13)')
parser.add_argument('--config-name', default='layer13_largemargin', type=str, metavar='CONFIG',
                    help='config name in config file (default: layer13_largemargin)')
...
def main():
    ...
    for name in ('batch_size', 'lrs', 'momentum', 'weight_decay', 'nesterov', 'loss'):
    ...
    args.train_transform = cf.train_transform[args.dataset]
    args.test_transform = cf.test_transform[args.dataset]
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.dataset == 'cifar10':
        model = mymodels.__dict__[args.arch](num_classes=10, ptype='max')
    elif args.dataset == 'cifar100':
        model = mymodels.__dict__[args.arch](num_classes=100, ptype='max')
    print(model)
