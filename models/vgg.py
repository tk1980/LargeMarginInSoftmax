...
def _vgg(arch, cfg, batch_norm, bow, **kwargs):
    seqs = list(cfgs[cfg])
    if not batch_norm:
    ...
    if bow:
        seqs[-1] = 'W'
        model = VGGbow(make_layers(seqs, batch_norm=batch_norm), **kwargs)
    else:
        model = VGG(make_layers(seqs, batch_norm=batch_norm), **kwargs)
    return model
